...
data "azurerm_subnet" "env_subnets" {
  resource_group_name  = data.azurerm_resource_group.main_rg.name
...
resource "azurerm_public_ip" "vm_pub_ip" {
  ...
  name                = "${local.std_prefix}-${var.vm.name}-${format("%02s", count.index+1)}-ip"
  resource_group_name = data.azurerm_resource_group.main_rg.name
...
resource "azurerm_network_interface" "vm_nic" {
  count = var.number_of_vm
  name                = "${local.std_prefix}-${var.vm.name}-${format("%02s", count.index+1)}-nic"
  location            = data.azurerm_resource_group.main_rg.location
  resource_group_name = data.azurerm_resource_group.main_rg.name
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.dev01vm_pub_ip[count.index].id
  }
}

resource "azurerm_network_security_group" "nsg_ssh" {
  name                = "${local.std_prefix}-${var.vm.name}-nsg"
  location            = data.azurerm_resource_group.main_rg.location
  resource_group_name = data.azurerm_resource_group.main_rg.name
...
resource "azurerm_linux_virtual_machine" "vm" {
  count = var.number_of_vm
  name                = "${local.std_prefix}-${var.vm.name}-${format("%02s", count.index+1)}-vm"
  resource_group_name = data.azurerm_resource_group.main_rg.name
  location            = data.azurerm_resource_group.main_rg.location
  size                = "Standard_B2s"
  ...
  os_disk {
    name = "${local.std_prefix}-${var.vm.name}-${format("%02s", count.index+1)}-osdisk"
    caching              = "ReadWrite"
